...
let
  ...
  # Minimal default theme — lets the terminal's own colours do the work.
  # No Powerline separators, no background fills, no Nerd Font glyphs required.
  defaultSettings = {
    format = lib.concatStrings [
      "$username"
      "$directory"
    ...
    username = {
      show_always = false; # only shown when relevant (SSH, root, etc.)
      format = "[$user]($style) ";
    };
    ...
    c       = { format = "via [$symbol($version )]($style)"; };
    ...
    python  = { format = "via [$symbol($version )(\\($virtualenv\\) )]($style)"; };
...
in
{
  programs.starship = {
    ...
    settings = if usePowerlineTheme then powerlineSettings else defaultSettings;
  };
}
